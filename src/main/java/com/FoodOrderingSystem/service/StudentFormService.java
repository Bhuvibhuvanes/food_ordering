...
public class StudentFormService {
	...
	public List<UsersForm> getAllStudents() {
		List<UsersForm> sf = studentRepo.findAll();
	...
	public Blob getStudentPhotoByStudentId(int id) throws SQLException {
		Optional<UsersForm> student = studentRepo.findById(id);
		if (student.isEmpty()) {
	...
	public UsersForm addStudent(MultipartFile file, long rfid_Number, String first_name, String last_name,
		...
		UsersForm s1 = studentRepo.save(s);
		return s1;
	}
	...
	public void updateStudentFields(UsersForm student, Map<String, Object> fields)
	...
	public UsersForm saveStudent(UsersForm student) {
		return studentRepo.save(student);
	}

	public UsersForm getStudentById(int student_id) {
		Optional<UsersForm> s = studentRepo.findById(student_id);
		if (s.isPresent()) {
			UsersForm f = s.get();
			return f;
		} else {
